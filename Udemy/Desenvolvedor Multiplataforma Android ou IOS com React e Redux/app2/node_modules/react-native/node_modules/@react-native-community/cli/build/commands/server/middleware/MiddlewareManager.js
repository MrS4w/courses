...
var _logToConsoleMiddleware = _interopRequireDefault(require("./logToConsoleMiddleware"));
...
class MiddlewareManager {
  constructor(options) {
    ...
    this.app = (0, _connect().default)().use(_getSecurityHeadersMiddleware.default).use(_loadRawBodyMiddleware.default).use((0, _compression().default)()).use('/debugger-ui', (0, _serveStatic().default)(debuggerUIFolder)).use((0, _openStackFrameInEditorMiddleware.default)(this.options)).use(_openURLMiddleware.default).use(_logToConsoleMiddleware.default).use(_copyToClipBoardMiddleware.default).use(_statusPageMiddleware.default).use(_systraceProfileMiddleware.default).use(_indexPage.default).use((0, _errorhandler().default)());
  }
